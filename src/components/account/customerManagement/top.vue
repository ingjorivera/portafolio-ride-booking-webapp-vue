...
function deleteClient() {
  clientsStore.deleteClient(clientsStore.selectedClientId || 0).then(() => {
...
function filter(value) {
  if (value.length > 2) {
    ...
    clientsStore.searchKeyword = value
    clientsStore.currentPage = 1
    //uiStore.scroll.resume()
...
<template>
  ...
  >
    <div class="title">{{ $t('texts.my_clients') }}</div>
